...
const Navbar: React.FC = () => {
  ...
  const navigate = useNavigate();
  const { user, isAdmin, signOut } = useAuth();
  const { isDark } = useTheme(); // Only used for context consistency
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            {user ? (
              ...
              <div className="hidden lg:flex items-center space-x-3">
                <Link to="/login" className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-white/90 hover:text-gray-900 dark:hover:text-white transition-colors">
                  Login
                </Link>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Link to="/register" className={`px-6 py-2 bg-gradient-to-r ${currentThemeConfig.gradient} backdrop-blur-sm text-gray-800 rounded-xl font-medium text-sm hover:shadow-lg transition-all duration-200 border border-white/30`}>
                    Sign Up
                  </Link>
                </motion.div>
              </div>
